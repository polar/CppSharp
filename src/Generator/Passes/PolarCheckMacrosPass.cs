...
namespace CppSharp.Passes
{
    ...
    /// </summary>
    public class PolarCheckMacroPass : TranslationUnitPass
    {
        private readonly string Prefix;

        public PolarCheckMacroPass(string prefix = "CS")
        {
            Prefix = prefix;
        }
        ...
        public override bool VisitParameterDecl(Parameter parameter)
        {
            ...
            /*
             * This is unfortunate. The preprocessentities seem to collect as the parameter list is parsed to the right.
             * So, we'll look at only the last expasion.
             * This already as the effect that if CS_OUT is specified, it is specified for all the rest of the parameters.
             * We mandate that you must issue a CS_IN in any parameter following if it is to change.
             */
            if (expansions.Count() > 0)
            {
                var expansion = expansions.Last();
                
                if (expansion.Text == Prefix + "_OUT")
                    parameter.Usage = ParameterUsage.Out;

                if (expansion.Text == Prefix + "_IN_OUT")
